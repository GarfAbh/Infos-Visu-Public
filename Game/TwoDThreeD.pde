  ...
  public TwoDThreeD(int width, int height) {
   ...
   double[][] solveExtrinsicMatrix(List<PVector> points2D) {
    ...
    && i < 4;i++){
        ...
        projectedCorners[i] = Mat.multiply(invK,new float[]{points2D.get(i).x,points2D.get(i).y,1});
    }
...
public static List<PVector> sortCorners(List<PVector> quad){
  ...
  public int compare(PVector b, PVector d){
    ...
    you can use Collections.roate to shift the corners inside the quad.
  */
  ...
  if(corner != 0){
    Collections.rotate(quad,-corner-1);
  }
  return quad;
}