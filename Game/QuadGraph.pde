...
class QuadGraph {
  ...
  int[][] graph;
  List<PVector> lines; // The lines have useful information for finding new cycles when using isConvex, intersect etc
  ...
  List<int[]> findCycles(float areaMin, float areaMax) {

    cycles.clear();
    for (int i = 0; i < graph.length; i++) {
      for (int j = 0; j < graph[i].length; j++) {
        findNewCycles(new int[] {graph[i][j]}, areaMin, areaMax);
      }
    }
    for (int[] cy : cycles) {
      String s = "" + cy[0];
  ...
  void findNewCycles(int[] path, float areaMin, float areaMax)
  {
    int n = path[0];
    int x;
          ...
          //  edge refers to our current node
        {
          ...
          } else if ((path.length > 3) && (x == path[path.length - 1]))
          {
            ...
            if (isNew(p) && isNew(inv)
                && isConvex(lines.get(p[0]), lines.get(p[1]), lines.get(p[2]), lines.get(p[3]))
                && nonFlatQuad(lines.get(p[0]), lines.get(p[1]), lines.get(p[2]), lines.get(p[3]))
                && validArea(lines.get(p[0]), lines.get(p[1]), lines.get(p[2]), lines.get(p[3]), areaMax, areaMin))
             {
              cycles.add(p);
            }
          }
  ...
  List<PVector> sortCorners(List<PVector> quad) {
    ...
    final PVector center = new PVector((a.x+b.x)/2, (a.y+b.y)/2);

    Collections.sort(quad, new Comparator<PVector>(){
      @Override
